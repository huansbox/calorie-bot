...
- [x] 純函式無 I/O，回傳含 action 與（若有）告警/提醒訊息的決策結構
- [x] 真值表六條分支全部實作
- [x] 「值相同」採嚴格相等（一位小數）
- [x] ±3kg 為閾值，同時涵蓋離譜壞值與真實大跳變
- [x] 單元測試覆蓋每條分支 + 邊界（差=3.0 不觸發、=3.01 觸發、負向跳變 −3.01 觸發）
- [x] 測試風格比照 `tests/test_dates.py` / `tests/test_backfill.py` 純函式真值表
