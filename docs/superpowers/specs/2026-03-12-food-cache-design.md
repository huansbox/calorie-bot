...
**B) `/f` 指令手動建立**
- `/f 公司便當 800 30 90 25` — 新增快取（格式同 `@` 手動輸入）
- 適用於 AI 沒分析過但你想預設的食物
...
- `calories` 存計算後的值（由 protein_g、carbs_g、fat_g 算出），方便列表顯示
...
- `handlers/meal.py` — `_process_food` 回覆加上 Inline Button
- `handlers/manual_meal.py` — `handle_manual_meal` 回覆加上 Inline Button
