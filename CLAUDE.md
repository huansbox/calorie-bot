...
- **密鑰管理**: 1Password — 本機 `op run` + VPS Service Account，`.env` 只存 `op://` 參照。`Developer / Calorie Bot` item 含 `TELEGRAM_TOKEN` / `SUPABASE_KEY` / `GEMINI_API_KEY` / `ANTHROPIC_API_KEY` / `COROS_EMAIL` / `COROS_PASSWORD`
